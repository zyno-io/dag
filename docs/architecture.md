...
| `packages/server` | HTTP API server and background worker         |
...
5. Stages the chart file to disk
6. Queues a **DeploymentJob** via BullMQ

### 3. Worker Processes Deployment

The background worker picks up the job and runs through these stages:
...
Each status change is broadcast via Redis so that SSE clients receive real-time updates.
